...
{
    const API_KEY = 'sk_test_51OQVa6BP9Ul0tUAlkpHvmQzKlPekUQdVmnoR2i8DojdcUi9eSdbOacHhV2whUpUSGQtnzPZOHndSTkKAkrCiKxRv004OgWLspB';
    //
    public function AddToCart(Request $request,$id)
    {
        ...
        //Check the course is already in cart
        $cartItem = Cart::search(function($cartItem, $rowId) use ($id){
           return $cartItem->id === $id;
        });
        if($cartItem->isNotEmpty()) {
            return response()->json(['error'=>'Course has already in your cart']);
        }
        ...
        if($course->discount_price == NULL){
            Cart::add([
                ...
                'options' => [
                    ...
                    'slug' => $request->get('course_name_slug'),
    ...
    public function CartRemove($rowId) {
        ...
        return response()->json(['success' => 'Remove Course from Cart successfully']);
    }

    public function CouponApply(Request $request){

        $coupon = Coupon::where('coupon_name',$request->get('coupon_name'))->where('coupon_validity','>=', Carbon::now()->format('Y-m-d'))->first();
    ...
    public function Payment(Request $request){
        ...
        $data->phone = $request->get('phone');
        $data->address = $request->get('address');
        $data->cash_delivery = $request->get('cash_delivery');
        ...
        $data = [
            ...
            Mail::to($request->get('email'))->send(new Orderconfirm($data));
            //Send notfication
