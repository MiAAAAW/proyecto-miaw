...
//                                    $lectures = App\Models\CourseLecture::where('section_id',$sec->id)->get();
                                $lectures = \Illuminate\Support\Facades\DB::select('SELECT * FROM course_lectures WHERE section_id = ?', [$sec->id]);
                                @endphp
                            <div class="card">
                                    ...
                                    <div class="card-body p-0">
                                        <ul class="curriculum-sidebar-list">
                                            ...
                                            <li class="course-item-link active">
                                                <div class="course-item-content-wrap">
                                                    <div class="custom-control custom-checkbox">
                                                        <input type="checkbox" name="checkbox_state" class="custom-control-input lecture-checkbox" id="courseCheckbox{{$lect->id}}" data-lecture-id="{{$lect->id}}" {{ $lect->checkbox_state ? 'checked' : '' }}>
                                                        <label class="custom-control-label custom--control-label" for="courseCheckbox{{$lect->id}}"></label>
                                                    </div><!-- end custom-control -->
                                                    <div class="course-item-content lecture-title" data-video-url="{{$lect->url}}" data-content="{!! $lect->content!!}">
...
<script>
    document.querySelectorAll('.lecture-checkbox').forEach((checkbox) => {
        checkbox.addEventListener('change',function(){
            ...
            fetch(`/lectures/update-checkbox-state/${lectureId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrfToken
                },
                body: JSON.stringify({ checkbox_state: isChecked }),
